...
import { View, Text, KeyboardAvoidingView, TouchableOpacity, TextInput, ActivityIndicator, Alert } from "react-native";
...
export const SignupTab = () => {
  ...
  const onSignupPress = async () => {
    ...
    if (!validateEmail.test(email)) return setValidationError({
      email: "Please enter a valid email address.",
      password: ""
    });
    if (!password) return setValidationError({
      email: "",
      password: "Please enter a valid password"
    });
    if (password !== confirmPassword) return setValidationError({
      email: "",
